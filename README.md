...
> **Агентам:** если задача — сделать `.dbml` по схеме другого проекта и открыть его здесь, прочитай [`AGENTS.md`](./AGENTS.md) и следуй ему. Краткий указатель — [`llms.txt`](./llms.txt). Не пиши полный DBML dbdiagram.io.
...
9. **Разложить** (в редакторе) — расставляет зоны и таблицы заново: меньше пересечений связей, меньше линий сквозь таблицы и пустого места в зонах. Это один шаг истории: «Отменить» возвращает прежнюю раскладку целиком. Стороны связей раскладка задаёт сама. В подсказке внизу — сколько пересечений было и стало.
10. **Все столбцы** в просмотре показывают скрытые поля. Выросшие таблицы раздвигают соседей по столбцу, зона вытягивается, зоны ниже сдвигаются. Это только отображение: координаты в файле не меняются, без галочки всё встаёт на место.
11. Длинные имена таблиц, полей и типов обрезаются многоточием, полное имя — во всплывающей подсказке; в PNG — так же.
...
- `Table` и поля вида `имя тип` с флагами `[pk]`, `[unique]`, `[hidden]` и заметкой `[note: 'текст']`
- однострочная `Note: 'текст'` у таблицы
- `TableGroup` — это зона на доске
- `Ref:` между полями, опционально `[delete: restrict|cascade|set null|...]`; `a.x < b.y` читается как `b.y > a.x`
...
## Схема из другого проекта

Коллеге skill ставить не нужно. Достаточно ссылки на этот репозиторий — агент сам читает `AGENTS.md` и `llms.txt` в корне.

```
Сделай schema.dbml по текущей схеме для редактора https://github.com/amkmonk/dbml-editor
```

Агент берёт таблицы из кода этого проекта (Prisma, миграции, модели) и пишет диалект из `AGENTS.md`. Готовый файл открывается здесь через **Открыть**.

## CLI

Те же раскладка и проверки — без браузера, из терминала. Нужен только Node.

```bash
node bin/dbml-editor.js help
node bin/dbml-editor.js check schema.dbml     # ошибки, предупреждения, пересечения
node bin/dbml-editor.js stats schema.dbml     # пересечения связей и линии сквозь таблицы
node bin/dbml-editor.js layout schema.dbml    # «Разложить», пишет файл на месте (или --out)
```

Обновить `.dbml` по живой базе PostgreSQL: выгрузить схему запросами из `sql` и передать выгрузку в `sync`. Заметки, скрытие, зоны и раскладка сохраняются; новые таблицы попадают в зону `--zone` (по умолчанию «новые»).
...
node bin/dbml-editor.js sync schema.dbml --columns columns.tsv --fks fks.tsv [--keep-hidden] [--layout]
node bin/dbml-editor.js check schema.dbml
```

`--keep-hidden` оставляет скрытые строки, которых нет в базе (например, записи индексов), `--layout` сразу раскладывает доску. Команды пишут файл в каноническом виде редактора — как «Скачать → DBML».
